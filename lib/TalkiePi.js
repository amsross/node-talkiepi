...
TalkiePi.prototype.init = function() {
  ...
  this.client.removeListener("error", h.log.bind(h, "talkiepi: error:"));
  this.client.on("error", h.log.bind(h, "talkiepi: error:"));

  this.client.removeListener("disconnect", h.log.bind(h, "talkiepi: disconnect:"));
  this.client.on("disconnect", h.log.bind(h, "talkiepi: disconnect:"));
  ...
  h(this.micInstance.getAudioStream())
    .tap(this.transmitStop.bind(this))
    .filter(x => this.buttonSPST.isDown(this.buttonSPST.value))
